...
public class Parser extends MainComplier{
    ...
    public ArrayList<String> readFile(File file) {
        ...
        try {
            ...
            if (choice == 1) {
                br = new BufferedReader(new FileReader(path+"a1.txt"));
            } else if (choice == 2){
                br = new BufferedReader(new FileReader(path+"a2.txt"));
            } else if (choice == 3){
                br = new BufferedReader(new FileReader(path+"a3.txt"));
            } else if (choice == 4){
                br = new BufferedReader(new FileReader(path+"a4.txt"));
            }
            String s = null;
    ...
    private void reformMap() {
        ...
        while (it.hasNext()) {
            ...
            ArrayList<ArrayList<String>> newLeftNew = new ArrayList<>();// 存放新的左边的新的右边
        ...
        while(isChange){
            ...
            while (it.hasNext()){
                ...
                for (int i = 0; i < lists.size(); i++) {
                    ArrayList<String> listCell = lists.get(i);
                    for (int j = 0; j < listCell.size(); j++) {
                        String v = listCell.get(j);
                        if(VN.contains(v)){
                            if(j+1 >= listCell.size()){
                                continue;
                            }
                            HashSet<String>follow = new HashSet<>();
                            if(keyFollow.containsKey(v)){
                                follow = keyFollow.get(v);
                            }
                            int followsize = follow.size();
                            if(VT.contains(listCell.get(j+1))){
                                follow.add(listCell.get(j+1));
                            }
                            else{
                                if(FIRST.get(listCell.get(j+1)).contains("ε") == false){
                                    follow.addAll(FIRST.get(listCell.get(j+1)));
                                }
                                else{
                                    Set<String> set = FIRST.get(listCell.get(j+1));
                                    set.remove("ε");
                                    follow.addAll(set);
                                    if(keyFollow.containsKey(listCell.get(j+1))){
                                        follow.addAll(keyFollow.get(listCell.get(j+1)));
                                    }
                                }
                            }
                            if(followsize!=follow.size()){
                                isChange = true;
                                keyFollow.put(v,follow);
                            }
                        }
                    }
                }
                    ...
                    for (int j = 1; j < FORM.length; j++) {
                        for (int k = 1; k < FORM[0].length; k++) {
                            if (FORM[j][0].equals(FORM[i][0]) && FORM[0][k].equals(vt)) {
                                FORM[j][k] = oneLeftFirst.get(oneLeftKey);//Vn->ε
                            }
                        }
                    }
                }
