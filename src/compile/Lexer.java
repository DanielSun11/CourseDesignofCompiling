...
public class Lexer extends MainComplier{
    ...
    //词法分析解释规则,定义字符数组
    String[] keyWords = {"if", "else", "while", "for","read", "write", "int", "double","float","public","static","void","main","class"};  //关键字数组
    ...
    public List<TokenList> getTokens() {
        ...
        while (totalStrt.hasMoreTokens()) {
            List<String> Tokens = new ArrayList<>();//行记号,一行中所有的Token
        ...
        for (int i = 0; i < tokenLists.size(); i++) {
            ...
            while(j<tokenList.size())
            {
                ...
                {
                    tokenList.remove(j);//删掉，后面自动补齐；j值不需要变
                }
                else
                {
                    j++;//识别下一个token
        ...
        for (int i = 0; i < multiComments.size(); i = i + 2)//去除多行注释中的整行注释
        {
            if ((multiComments.size() % 2) == 0 && i <= multiComments.size() - 2)//判断注释是否未闭合
            {
                if (multiComments.get(i).getComment().equals("/*") && multiComments.get(i + 1).getComment().equals("*/")) {
                    int tempj = multiComments.get(i).getRow();
                    for (int j = multiComments.get(i).getRow() ; j <=multiComments.get(i + 1).getRow(); j++) {
                        tokenLists.remove(tempj);
                    }
    ...
    public void Analysis() {
        ...
        for (int i = 0; i < tokenLists.size(); i++) {
            ...
            int lineid = tokenLists.get(i).getRow();
            outText.append("--------------------------------------------------分析第" + (lineid + 1) + "行--------------------------------------------------" + "\r\n");
            for (int j = 0; j < tokenList.size(); j++) {
                ...
                if (tokenList.get(j).matches(RegexToId) && (Count == 0)) {
                    ...
                    outText.append(tokenList.get(j) + " 是赋值号" + "\r\n");
                } else if (Count == 0) {
